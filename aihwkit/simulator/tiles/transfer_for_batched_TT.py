...
class BatchedTransferSimulatorTile(SimulatorTile, Module):
    ...
    def __init__(
        ...
        if not isinstance(rpu_config.device, TransferCompound):
            raise ConfigError("Device chould be of type TransferCompound")
        ...
        rpu_config_0 = SingleRPUConfig(
            ...
            tile_class=AnalogTile#WithoutPeriphery,
        )
    ...
    #     # Ensure we have enough tiles for the batch
    #     assert in_batch_size <= self.num_tiles, \
    #         f"Not enough tiles ({self.num_tiles}) for batch size {in_batch_size}"
        
    #     # Process tiles in parallel using a list comprehension
    #     outputs = torch.stack([
    #         weight_tile.tile.forward(
    #             x_input[i].unsqueeze(0), bias, in_trans, out_trans, is_test, non_blocking
    #         ) 
    #         for i, weight_tile in enumerate(self.weight_tiles[:in_batch_size])
    #     ])
        
    #     return outputs.view(in_batch_size, -1)
    ...
    #             d_input[i].unsqueeze(0), bias, in_trans, out_trans, non_blocking
    #         )
    #         for i, weight_tile in enumerate(self.weight_tiles[:d_batch_size])
    #     ])

    #     return outputs.view(d_batch_size, -1)
    ...
    def update(
        ...
        if rest_count >= transfer_every:
            ...
            write_values = -self.learning_rate * 10 * omega
