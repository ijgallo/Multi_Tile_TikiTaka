...
class BatchedTransferSimulatorTile(SimulatorTile, Module):
    ...
    def __init__(
        ...
        self.lr_w = self.device_config.step * self.to_weight_granularity
        [weight_tile.set_learning_rate(self.lr_w) for weight_tile in self.weight_tiles]
        ...
        self.granularity = rpu_config.update.desired_bl * self.from_weight_granularity
        self.lr_a_auto_scale = cfg.fast_lr * self.granularity
    ...
    def forward(
        ...
        Args:
            ...
            multi: Whether to pass the same inputs to multiple tiles
        ...
        self.bm_rounds.append(bm_rounds)
    ...
    def update(
        ...
        if rest_count >= transfer_every:
            ...
            if cfg.in_chop_prob:
                if cfg.in_chop_random:
                    switch_chopper = rand(1).item() < cfg.in_chop_prob
                                    # Log random chopper switch check
            ...
            # BL = torch.ceil(self.lr_w * torch.abs(write_values[:, None]) * torch.abs(self._transfer_vec[k][None, :]) / self.granularity)
            # BL  = torch.minimum(BL,torch.tensor(self.desired_bl));
            # print("Number of weights updated:", (BL > 0).sum().item())
            # print("Number of weights at max pulses (31):", (BL == 31).sum().item())
            # print("Number of weights not updated (0):", (BL == 0).sum().item())
            # print("Max number of pulses needed :", BL.max().item()) 
            # self.max_pulses.append(BL.max().item())
            ...
            self.dlog.append(self.create_log_entry(
                "digital_la", "sum", "update", "O(n)", "medium",
                vector_length=write_values.numel(), times=1
            ))
            self.dlog.append(self.create_log_entry(
                "digital_la", "comparison", "update", "O(1)", "very_low", times=1
            ))
            # write to weight
            ...
            self.dlog.append(self.create_log_entry(
                "digital_la", "conditional", "update", "O(1)", "very_low", times=13
            ))

            self.dlog.append(self.create_log_entry(
                "digital_la", "arithmetic", "update", "O(1)", "very_low", times=5
            ))
    ...
    def set_weights(self, weights: Tensor) -> None:
        ...
        if weights.dim() == 2:
            for weight_tile in self.weight_tiles:
                weight_tile.tile.set_weights(weights)
        else:
    ...
    def get_hidden_parameter_names(self) -> List[str]:
        ...
        for i, w_tile in enumerate(self.weight_tiles, 1):
            ...
            names_1.extend([f"{n}_{i}" for n in tile_names])
    ...
    def load_extra(self, extra: Dict, strict: bool = False) -> None:
        ...
        if "grad_tile" not in extra or "weight_tiles" not in extra:
