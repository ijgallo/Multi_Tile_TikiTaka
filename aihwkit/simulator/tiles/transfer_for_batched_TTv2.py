...
class BatchedTransferSimulatorTile(SimulatorTile, Module):
    ...
    def __init__(
        ...
        self.update_bl = rpu_config.update.desired_bl
    ...
    def update(
        ...
        if rest_count >= transfer_every:
            ...
            # BL = torch.ceil(self.learning_rate * torch.abs(write_values[:, None]) * torch.abs(self._transfer_vec[k][None, :]) / self.granularity)
    ...
    def program_weights(
        ...
        for weight_tile, target_weights in zip(self.weight_tiles, weights):
            ...
            for _ in range(max_iter):
                y = weight_tile.tile.forward(x_values, False)
                error = y - target_values
                if tolerance is not None and (error.abs().mean().item() / target_max) < tolerance:
                    break
                weight_tile.tile.update(x_values, error, False)  # type: ignore
