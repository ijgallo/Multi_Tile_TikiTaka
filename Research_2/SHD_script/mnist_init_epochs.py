...
def train(train_set, validation_set, model, optimizer, scheduler, classifier):
    """Train the network.
    ...
    while len(a) == 0 or a[-1] < Target_accuracy:
        ...
        loss, acc = test_evaluation(model, validation_set, classifier)
        ...
        # if acc > 0.90 and scheduler.get_last_lr()[0] >= 0.1 * 31 \
        #     or acc > 0.95 and scheduler.get_last_lr()[0] >= 0.05 * 31:
        #     print("Decaying learning rate from {} to {}".format(scheduler.get_last_lr()[0], scheduler.get_last_lr()[0] * 0.5))
        #     scheduler.step()
        if acc > 0.95 and scheduler.get_last_lr()[0] >= 0.1 * 31:
            print("Decaying learning rate from {} to {}".format(scheduler.get_last_lr()[0], scheduler.get_last_lr()[0] * 0.5))
            scheduler.step()
...
if __name__ == "__main__":
    ...
    num_tiles = [32, 64]#1, 4, 8, 16, 
    batch_size = 64
    init_std = [0.0, 0.02]#, 0.04, 0.06, 0.08, 0.1
    ...
    for nt in num_tiles:
        ...
        for istd in init_std:
            ...
            print(f'Mean Digital Accuracy: {np.mean(accuracy):.2f} for num tiles {nt} and init_std {istd}')
            results.append({
                'num_tiles': nt,
                'init_std': istd,
            ...
            df.to_csv('mnist_agad_init_epochs_v1.csv', index=False)